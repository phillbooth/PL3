# LO Non-Supported Native Features

## Summary

LO is a strict, memory-safe, security-first general-purpose programming language.

This document defines what LO should **not** support as native language features.

The purpose of this document is to prevent LO from becoming too large, unclear, unstable, or framework-like.

LO should provide safe primitives.
...
LO may provide the safe building blocks.

LO should not provide the full specialist system.

---

## Core Non-Support Principle

LO should not natively become:
...
LO should not natively support:
...
```text
LO supports safe primitives that search packages can use.
```
...
```text
LO supports binary and stream primitives for image packages.
```
...
```text
LO supports safe streaming primitives that video packages can use.
```
...
```text
LO supports file and stream primitives. Document-specific handling belongs in packages.
```

Avoid wording:

```text
LO natively supports PDF, DOCX and XLSX.
```
...
```text
LO natively supports email sending.
```
...
```text
LO natively supports SQL, NoSQL and vector databases.
```
...
```text
LO supports primitives that queue packages and worker frameworks can use.
```

Avoid wording:

```text
LO natively supports queues.
```
...
```text
LO natively supports the browser DOM.
```
...
```text
LO natively supports push notifications.
```
...
```text
LO natively supports accessibility scanning.
```
...
```text
LO can be AI-friendly without becoming an AI platform.
```

Avoid wording:

```text
LO natively supports AI agents.
```
...
```text
LO can compile for cloud, server, edge, worker and CLI profiles without becoming a cloud platform.
```

Avoid wording:

```text
LO natively supports AWS, Azure, Google Cloud or Firebase.
```
...
```text
LO natively supports Cloudflare Workers.
```
...
```text
LO natively supports VAT, payroll or invoicing.
```
...
LO should not hard-code providers such as:
...
```text
LO natively supports Stripe, Firebase or AWS.
```
...
LO should not natively support hard-coded lists such as:
...
```text
LO has built-in supported translation languages.
```
...
```text
CMS functionality belongs in frameworks or applications built with LO.
```

Avoid wording:

```text
LO includes CMS features.
```
...
```text
LO natively supports UI components.
```
...
LO should not become an operating system.
...
LO may provide controlled access to:
...
```text
LO can build system-aware software without becoming an operating system.
```

Avoid wording:

```text
LO includes operating system features.
```
...
LO should not hard-code one vendor or hardware platform.

LO should not natively support only:
...
```text
LO natively supports one specific chip vendor.
```
...
```text
LO natively supports media hosting.
```
...
```text
LO natively supports analytics tracking.
```
...
```text
LO natively supports S3 or Firebase Storage.
```
...
```text
LO natively manages deployments.
```
...
```text
LO supports package safety, but registry hosting is separate infrastructure.
```

Avoid wording:

```text
LO includes its own package marketplace.
```
...
```text
LO natively supports legal, medical or financial decisions.
```
...
```text
Game engines can use LO primitives, but they are not native language features.
```

Avoid wording:

```text
LO includes a built-in game engine.
```
...
```text
LO natively supports data science notebooks.
```
...
Before adding any feature to LO itself, ask:
...
Would adding this make LO harder to keep stable?
Would this create provider lock-in?
Would this make LO look like a framework rather than a language?
```
...
```text
LO supports file streams and binary data that PDF packages can use.
```
...
```text
LO supports networking, safe secrets and typed payloads that email packages can use.
```
...
```text
LO supports typed database driver boundaries. SQL drivers belong in packages.
```
...
```text
LO supports safe compute, networking and streaming primitives that AI packages can use.
```
...
```text
LO supports binary streams and backpressure primitives that video packages can use.
```
