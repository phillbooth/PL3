# LO Video Package Boundaries and Compute Auto
...
boot.lo
main.lo
video-provider-example.lo
video-policy.lo
video-worker-example.lo
browser-video-example.lo
```
...
This document defines how LO should safely support **video packages, video providers and video AI workflows** without making video processing a native language feature.
...
LO should provide safe primitives that video packages can use:
...
```text
LO natively supports video processing.
LO natively supports video classification.
LO natively supports video search.
LO natively supports video embeddings.
LO natively supports video transcription.
LO natively supports camera recording.
LO natively supports screen capture.
LO natively supports video moderation.
```

LO should say:

```text
LO supports safe typed boundaries that video packages can use.
```
...
These should be package-defined or standard-library-candidate types, not required LO core language types.
...
LO should provide the safety model around these capabilities.
...
They should not imply that LO core contains a native video engine.
...
LO may support configuration validation, reports, effects and permission checks.
...
Video classification is not a native LO feature.
...
LO should make memory, target and permission behaviour visible.
...
Video transcription is a workflow, not a native LO feature.
...
LO should report the stages.

LO should not make video transcription a built-in language task.
...
LO provides typed boundaries and reports.
...
LO should not provide the summarisation model natively.
...
LO handles safety, typing, effects and reports.
...
LO should not natively implement thumbnail generation.
...
LO should not natively implement video demuxing or codec logic.
...
LO should support typed decisions and audit reports.
...
Video search is not a native LO feature.
...
LO should not natively implement browser video APIs.
...
LO should support primitives and reports that help packages enforce:
...
Video streaming should use LO's general streaming primitives.
...
LO provides the general stream safety model.
...
Queues are not native LO platforms.

LO may support worker profiles and typed job boundaries.
...
This should be treated as interop, not native LO video support.
...
LO should support reports that make video package behaviour visible.
...
```json
{
  "videoPackageReport": {
    ...
    "flow": "classifyVideo",
    "source": "src/video/classify.lo:4",
    "input": {
      "type": "FileRef",
...
AI note:
Keep camera access, screen capture, file loading, validation and permission checks outside model-heavy compute flows. Use video packages for classification, transcription, moderation and search. Do not treat video processing as a native LO feature.
```
...
```json
{
  "videoPackages": [
    ...
    },
    {
      ...
      "source": "src/video/transcribe.lo:4",
      "input": "FileRef",
      "output": "Transcript",
      "effects": ["file.read", "compute.run"],
      "computeMode": "auto"
...
LO should support rules and reports that help video packages enforce:
...
```text
turn LO into a video editing framework
turn LO into a video hosting platform
turn LO into a streaming platform
turn LO into a media player framework
turn LO into a transcoding platform
...
This document replaces the earlier idea that video should be a native LO media type and AI task area.

The revised position is:

```text
Video processing is not a native LO feature.
Video processing is a package/provider/framework area.
LO provides safe primitives that video packages can use.
```
...
```text
LO should natively support video as a first-class media type.
LO should natively support video classification.
LO should natively support object detection in video.
LO should natively support video segmentation.
LO should natively support scene detection.
LO should natively support motion detection.
LO should natively support activity recognition.
LO should natively support video summarisation.
LO should natively support video-to-text description.
LO should natively support video transcription.
LO should natively support subtitle generation.
LO should natively support video embeddings.
LO should natively support video similarity search.
LO should natively support thumbnail generation.
LO should natively support frame extraction.
LO should natively support audio extraction.
LO should natively support video moderation.
LO should natively support stream analysis.
LO should natively define Video.
LO should natively define VideoClip.
LO should natively define VideoStream.
LO should natively define VideoFrame.
LO should natively define VideoEmbedding.
LO should natively define video AI task types.
```

## What Was Kept

The following ideas remain valid, but should be described as general LO primitives:
...
LO should support video safely without becoming a video engine, video editor, streaming platform, hosting platform, video CMS, transcoding platform, media application, or provider SDK.
...
Video systems should be built on top of LO, not inside LO.
