...
This document describes the design direction for **LO / Logic Omni**.

LO is a strict, memory-safe, security-first, JSON-native, API-native and accelerator-aware programming language concept.

The design should make LO practical for normal software development today while preparing for future targets such as GPU, WebAssembly, photonic compute and ternary / 3-way logic systems.

---

## Design Summary

LO should be designed around the foLOwing idea:

> Write one safe, strict `.lo` source project and compile it into multiple target outputs.
...
LO should be useful without photonic hardware.
...
LO should not make futuristic hardware support more important than practical developer experience.
...
```text
boot.lo clearly means the project starts here
main.lo can still be used for simple scripts or small projects
```
...
LO should separate native LO packages from external vendor code.
...
LO should use clear, compact and predictable syntax.
...
LO should use `flow` as the primary unit of executable logic.
...
LO should use `Option<T>` for missing values.
...
LO should use `Result<T, E>` for operations that can fail.
...
LO should include a first-class decision type for 3-way business/security decisions.
...
LO should support `Tri` for more general 3-way logic.
...
LO should not allow non-boolean values inside `if`.
...
LO should use `match` for exhaustive state handling.
...
LO should support `await` for async operations.
...
LO should separate `await` from condition-based waiting.
...
LO should make backpressure explicit.
...
LO should support JSON policies.
...
LO should support streaming JSON.
...
LO should support partial decoding.
...
LO should support structured JSON/data transformation.
...
LO should support API contracts.
...
LO should support external API clients.
...
LO should include maths-first types.
...
LO should support permission declarations.
...
LO should make environment access explicit.
...
LO should compile to an intermediate representation before final outputs.
...
LO should generate source maps.

Source maps should connect compiled output back to original `.lo` files.
...
LO should generate machine-readable reports.
...
LO should generate a build manifest.
...
LO should support debug and release builds.
...
LO should support these targets:
...
LO should make fallback explicit.
...
LO documentation should be clear, structured and example-heavy.
...
LO should be designed as a practical language first and a future-hardware language second.
